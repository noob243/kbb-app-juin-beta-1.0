...
const AIAssistantPage: FC<AIAssistantPageProps> = ({ clients, cases, tasks, invoices }) => {
    ...
    return (
        <div className={`flex h-full max-h-screen -m-8 transition-colors duration-500 ${isDarkMode ? 'bg-gray-950 text-gray-100' : 'bg-white text-gray-900'}`}>
            {/* Sidebar History */}
            <div className={`w-72 border-r flex flex-col h-full overflow-hidden transition-all duration-300 ${isDarkMode ? 'bg-gray-900 border-gray-800 shadow-2xl' : 'bg-slate-100 border-slate-200 shadow-inner'}`}>
                <div className={`p-4 border-b transition-colors ${isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-slate-200'}`}>
                    <button 
                        onClick={handleNewChat}
                        className="w-full flex items-center justify-center space-x-2 bg-indigo-600 text-white py-2.5 rounded-xl hover:bg-indigo-700 transition-all hover:shadow-lg hover:shadow-indigo-500/30 font-semibold text-sm"
                    >
            ...
            <div className={`flex-1 flex flex-col min-w-0 transition-colors relative ${isDarkMode ? 'bg-gray-950 shadow-[inset_0_0_100px_rgba(0,0,0,0.2)]' : 'bg-white'}`}>
                ...
                <div className={`flex justify-between items-center p-6 border-b transition-all duration-300 ${isDarkMode ? 'border-gray-800 bg-gray-900/20' : 'border-slate-100'}`}>
                    <div className="flex flex-col">
                        <h1 className={`text-2xl font-black flex items-center tracking-tight ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>
                    ...
                    <div className="flex items-center space-x-3">
                        <button 
                            ...
                            className={`p-2.5 rounded-xl transition-all border hover:scale-105 active:scale-95 ${isDarkMode ? 'bg-gray-800 border-gray-700 text-yellow-400 hover:bg-gray-700 shadow-lg' : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-slate-100 shadow-sm'}`}
                        >
                            {isDarkMode ? (
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <button 
                            ...
                            className={`flex items-center space-x-2 px-4 py-2 text-xs font-bold rounded-xl transition-all hover:scale-105 active:scale-95 ${isDarkMode ? 'text-red-400 hover:bg-red-900/20' : 'text-red-600 hover:bg-red-50'}`}
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                            </svg>
                ...
                <div className={`flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar transition-colors duration-500 ${isDarkMode ? 'bg-gray-900/30' : 'bg-slate-50/20'}`}>
                    {messages.map((msg, idx) => (
                        <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`group relative flex max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                ...
                                <div className={`relative px-6 py-4 rounded-3xl shadow-xl transition-all border duration-300 flex-1 min-w-[200px] ${
                                    ...
                                    {msg.role === 'assistant' && editingIndex === null && (
                                        <div className="absolute -right-14 top-0 flex flex-col space-y-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                            <button 
                                                onClick={() => handleCopy(msg.content, idx)}
                                                className={`p-2 border rounded-xl shadow-lg transition-all hover:scale-110 ${isDarkMode ? 'bg-gray-800 text-gray-400 border-gray-700 hover:text-indigo-400' : 'bg-white text-gray-400 border-gray-100 hover:text-indigo-600'}`}
                                                title="Copier"
                                            >
                                                {copyFeedback === idx ? (
                                            ...
                                            <button 
                                                ...
                                                className={`p-2 border rounded-xl shadow-lg transition-all hover:scale-110 ${isDarkMode ? 'bg-gray-800 text-gray-400 border-gray-700 hover:text-yellow-400' : 'bg-white text-gray-400 border-gray-100 hover:text-indigo-600'}`}
                                            ...
                                            <div className="flex flex-col space-y-2">
                                                <button 
                                                    ...
                                                    className={`p-2 border rounded-xl shadow-lg transition-all hover:scale-110 ${msg.feedback === 'positive' ? 'bg-green-500/10 text-green-500 border-green-500/20' : (isDarkMode ? 'bg-gray-800 text-gray-400 border-gray-700 hover:text-green-500' : 'bg-white text-gray-400 border-gray-100 hover:text-green-600')}`}
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                ...
                                                <button 
                                                    ...
                                                    className={`p-2 border rounded-xl shadow-lg transition-all hover:scale-110 ${msg.feedback === 'negative' ? 'bg-red-500/10 text-red-500 border-red-500/20' : (isDarkMode ? 'bg-gray-800 text-gray-400 border-gray-700 hover:text-red-500' : 'bg-white text-gray-400 border-gray-100 hover:text-red-600')}`}
                                                >
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
